...
const InterviewDrawer = ({ open, onClose, studentId, recordId, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (!recordId) {
      ...
      fetch(`https//localhost/api/interviews/id/${recordId}`)
        .then(res => res.json())
        .then(data => {
          setFormData({
  ...
  const handleSave = async () => {
    ...
    const url = isUpdate
      ? `https//localhost/api/interviews/update/${formData.id}`
      : `https//localhost/api/interviews/insert`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await fetch(`https//localhost/api/interviews/delete/${formData.id}`, {
        method: "DELETE"
      });
      onSave(null);
      onClose();
