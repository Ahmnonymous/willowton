...
const StudentDetailDrawer = ({ open, onClose, studentId, onSave }) => {
    ...
    if (open) {
      if (studentId) {
        const fetchStudentData = async () => {
          try {
            const response = await fetch(`http://localhost/api/student-details/${studentId}`);
  ...
  const handleSave = async () => {
    const url = studentId
      ? `http://localhost/api/student-details/update/${studentId}`
      : `http://localhost/api/student-details/insert`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost/api/student-details/delete/${studentId}`,
        { method: "DELETE" }
      );
