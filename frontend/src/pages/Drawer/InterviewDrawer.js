...
const InterviewDrawer = ({ open, onClose, studentId, recordId, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (!recordId) {
      ...
      fetch(`http://localhost/api/interviews/id/${recordId}`)
        .then(res => res.json())
        .then(data => {
          setFormData({
  ...
  const handleSave = async () => {
    ...
    const url = isUpdate
      ? `http://localhost/api/interviews/update/${formData.id}`
      : `http://localhost/api/interviews/insert`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await fetch(`http://localhost/api/interviews/delete/${formData.id}`, {
        method: "DELETE"
      });
      onSave(null);
      onClose();
