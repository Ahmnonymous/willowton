...
const ParentsDrawer = ({ open, onClose, studentId, parentId, onSave }) => {
  ...
  useEffect(() => {
    if (open) {
      if (parentId) {
        fetch(`https//localhost/api/parents-details/id/${parentId}`)
          .then((res) => res.json())
  ...
  const handleSave = async () => {
    ...
    const url = isUpdate
      ? `https//localhost/api/parents-details/update/${formData.id}`
      : "https//localhost/api/parents-details/insert";
  ...
  const handleDelete = async () => {
    ...
    const res = await fetch(`https//localhost/api/parents-details/delete/${formData.id}`, {
      method: "DELETE",
    });
    if (res.ok) {
      onSave(null);
