...
const VoluntaryServiceDrawer = ({ open, onClose, studentId, recordId, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (!recordId) {
      ...
      fetch(`https//localhost/api/voluntary-service/id/${recordId}`)
        .then(res => res.json())
        .then(data => {
          setFormData({
  ...
  const handleSave = async () => {
    ...
    const url = isUpdate
      ? `https//localhost/api/voluntary-service/update/${formData.id}`
      : `https//localhost/api/voluntary-service/insert`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await fetch(`https//localhost/api/voluntary-service/delete/${formData.id}`, {
        method: "DELETE"
      });
      onSave(null);
      onClose();
  ...
  const handleViewFile = () => {
    if (formData.id) {
      window.open(`https//localhost/api/voluntary-service/view/${formData.id}`, "_blank");
    }
  };
