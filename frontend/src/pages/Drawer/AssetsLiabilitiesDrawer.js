...
const AssetsLiabilitiesDrawer = ({ open, onClose, studentId, assetLiabilityId, onSave }) => {
  ...
  useEffect(() => {
    if (open && assetLiabilityId) {
      fetch(`http://localhost:5000/api/assets-liabilities/id/${assetLiabilityId}`)
        .then(res => res.json())
        .then(data => {
          if (data) {
            setFormData({
  ...
  const handleSave = () => {
    const url = assetLiabilityId
      ? `http://localhost:5000/api/assets-liabilities/update/${assetLiabilityId}`
      : `http://localhost:5000/api/assets-liabilities/insert`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await fetch(`http://localhost:5000/api/assets-liabilities/delete/${assetLiabilityId}`, {
        method: "DELETE"
      });
      onSave(null);
