...
const AttachmentsDrawer = ({ open, onClose, studentId, attachmentId, onSave }) => {
  ...
  useEffect(() => {
    ...
    // Editing mode
    fetch(`https//localhost/api/attachments/id/${attachmentId}`)
      .then((res) => res.json())
      .then((data) => {
        setFormData({
  ...
  const handleSave = async () => {
    ...
    const url = isUpdate
      ? `https//localhost/api/attachments/update/${formData.id}`
      : `https//localhost/api/attachments/insert`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await fetch(`https//localhost/api/attachments/delete/${formData.id}`, {
        method: "DELETE",
      });
      onSave(null);
  ...
  const handleViewFile = () => {
    if (formData.id) {
      window.open(`https//localhost/api/attachments/view/${formData.id}`, "_blank");
    }
  };
