...
const StudentDetailDrawer = ({ open, onClose, studentId, onSave }) => {
  ...
  const handleSave = async () => {
    const url = studentId
      ? `http://localhost:5000/api/student-details/update/${studentId}`
      : `http://localhost:5000/api/student-details/insert`;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/student-details/delete/${studentId}`,
        { method: "DELETE" }
      );
